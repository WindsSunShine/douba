...
{
    //scrollView
    _bottomScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, 375, self.bounds.size.height)];
//    _bottomScrollView.backgroundColor = [UIColor cyanColor];
    ...
    //活动内容
    _contextLabel = [[UILabel alloc] initWithFrame:CGRectMake(20, 236, 335, 0)];
...
{
    //计算活动内容的高度
    CGRect contentRect = [content boundingRectWithSize:CGSizeMake(self.frame.size.width, 1000000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12.0]} context:nil];
    ...
    _bottomScrollView.contentSize = CGSizeMake(335, height);
...
- (void)setActivityModel:(ActivityModel *)activity {
    [self.activityImageView sd_setImageWithURL:[NSURL URLWithString:activity.image] placeholderImage:[UIImage imageNamed:@"picholder"]];
    ...
    self.ownerLabel.text = activity.owner;
    self.addressLabel.text = activity.address;
    self.contextLabel.text = activity.content;
    [self adjustSubviewsWithContent:activity.content];
}
